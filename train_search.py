...
parser.add_argument('--k', type=int, default=0.8, help='sample probility')
...
def main():
    ...
    model = Network(criterion, feat_data.shape[1], int(max(labels)) + 1, hidden_size, k=args.k, num_layers=args.num_layers,epsilon=args.epsilon, args=args)
    ...
    for epoch in range(args.epochs):
        ...
        train_auc, train_obj = train(args.data, data, edge_index,train_label_priors, valid_label_priors,model, architect, criterion, optimizer, lr)
        scheduler.step()
        t2 = time.time()
        search_cost += (t2 - t1)

        valid_auc, valid_obj = infer(args.data, data,edge_index, model, criterion)
        test_auc,  test_obj = infer(args.data, data,edge_index, model, criterion, test=True)
...
def train(dataset_name, data, edge_index, train_label_priors, valid_label_priors,model, architect, criterion, optimizer, lr):
