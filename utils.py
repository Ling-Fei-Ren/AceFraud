...
def sparse_to_adjlist(sp_matrix):
    ...
    edges=torch.tensor(edges,dtype=int)
    return edges

def sparse_to_adjlist_1(sp_matrix):
    """
    Transfer sparse matrix to adjacency list
    :param sp_matrix: the sparse matrix
    :param filename: the filename of adjlist
    ...
    # add self loop
    homo_adj = sp_matrix
    edges=torch.tensor(homo_adj,dtype=int)
    return edges

def FraudTeleminiDataset():
    tele = loadmat('data/tele_mini/tele_mini.mat')
    labels = tele['label'].flatten()
    feat_data = tele['features']
    net_ucu = tele['tele_ucu']
    net_usu = tele['tele_usu']
    net_utu = tele['tele_utu']
    net_ucu = sparse_to_adjlist_1(net_ucu)
    net_usu = sparse_to_adjlist_1(net_usu)
    net_utu = sparse_to_adjlist_1(net_utu)
    return [net_ucu, net_usu, net_utu], labels, feat_data

def FraudTelemaxDataset():
    tele = loadmat('data/tele_max/tele_max.mat')
    labels = tele['label'].flatten()
    feat_data = tele['features']
    net_ucu = tele['tele_ucu']
    net_usu = tele['tele_usu']
...
def FraudYelpDataset():

    yelp = loadmat('data/Yelp/YelpChi.mat')
    labels = yelp['label'].flatten()
    feat_data = yelp['features'].todense().A
    net_rur = yelp['net_rur']
    net_rtr = yelp['net_rtr']
    net_rsr = yelp['net_rsr']
    net_rur = sparse_to_adjlist(net_rur)
    net_rtr = sparse_to_adjlist(net_rtr)
    net_rsr = sparse_to_adjlist(net_rsr)
    return [net_rur,net_rtr,net_rsr],labels,feat_data

def FraudAmazonDataset():

    amz = loadmat('data/Amazon/Amazon.mat')
    ...
    net_upu=sparse_to_adjlist(net_upu)
    net_usu = sparse_to_adjlist(net_usu)
    net_uvu = sparse_to_adjlist(net_uvu)
    return [net_upu,net_usu,net_uvu],labels,feat_data
