...
class Architect(object):
    ...
    def step(self, data, edge_index,valid_label_priors,eta, network_optimizer, unrolled):
        self.optimizer.zero_grad()
        if unrolled:
            self._backward_step_unrolled(data, valid_label_priors,eta, network_optimizer)
        else:
            self._backward_step(data,valid_label_priors,edge_index, is_valid=True)
        self.optimizer.step()

    def _backward_step(self, data, valid_label_priors, edge_index,is_valid=True):
        if self.args.data == 'PPI':
            ...
            loss = self.model._loss(data,valid_label_priors,edge_index, is_valid)
            loss.backward()
